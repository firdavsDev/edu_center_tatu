...
class Alerter:
    ...
    def from_environment(cls) -> Alerter:
        try:
            ...
            raise KeyError(
                "ALERT_BOT_TOKEN must be set in environment variables") from e

        try:
            chat_id = os.environ["ALERT_CHAT_ID"]
        except KeyError as exc:
            raise KeyError(
                "ALERT_CHAT_ID must be set in environment variables") from exc
    ...
    def exception_alert(self, func):
        ...
        def inner(*args, **kwargs):
            try:
                ...
                text = f"<b>{type(e).__name__}('{e}')</b> in <u>{func.__name__}</u> from <u>{
...
# Create Class
tg_alert = Alerter(bot_token='6925484287:AAG9MB8h9Lo0XN_WzfNuzfZJ3-yDJijteus',
                   chat_id='-1002061753274')
# We use in:
# except Exception as e:
